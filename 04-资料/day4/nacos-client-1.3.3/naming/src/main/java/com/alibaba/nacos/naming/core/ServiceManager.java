...
public class ServiceManager implements RecordListener<Service> {
    ...
    public void createServiceIfAbsent(String namespaceId, String serviceName, boolean local, Cluster cluster)
        ...
        if (service == null) {
            ...
            if (cluster != null) {
                // 建立service与cluster间的关联关系
                cluster.setService(service);
                service.getClusterMap().put(cluster.getName(), cluster);
            }
            service.validate();
            // 将这个service写入到注册表，并同步给其它nacos
            putServiceAndInit(service);
            if (!local) {
    ...
    public void addInstance(String namespaceId, String serviceName, boolean ephemeral, Instance... ips)
        ...
        synchronized (service) {
            // 注册instance到注册表，返回最新的实例列表
            List<Instance> instanceList = addIpAddresses(service, ephemeral, ips);

            Instances instances = new Instances();
            instances.setInstanceList(instanceList);
            // 将更新过的instances同步到nacos集群
    ...
    public List<Instance> updateIpAddresses(Service service, String action, boolean ephemeral, Instance... ips)
        ...
        // 从nacos集群中获取指定service的数据
        Datum datum = consistencyService.get(KeyBuilder.buildInstanceListKey(service.getNamespaceId(), service.getName(), ephemeral));

        // 获取到当前service的所有instance
        ...
        if (datum != null) {
            // 将外来的数据写入到instanceMap
            instanceMap = setValid(((Instances) datum.value).getInstanceList(), currentInstances);
        } else {
            instanceMap = new HashMap<>(ips.length);
        }

        // 遍历所有要注册的instance，当然这里只有一个
        for (Instance instance : ips) {
            // 若当前servcie中不包含当前注册的instance的cluster，则创建新的cluster
